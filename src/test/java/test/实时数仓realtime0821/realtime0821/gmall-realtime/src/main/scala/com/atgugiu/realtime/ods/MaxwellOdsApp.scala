...
object MaxwellOdsApp extends BaseAppV1 {
    ...
    override val groupId: String = "MaxwellOdsApp"
    override val topics: Set[String] = Set("maxwell_gmall_db")
    ...
    override def run(ssc: StreamingContext,
        ...
        sourceStream
            .map(line => {
                ...
                (tablaName, operate, Serialization.write(dataValue))
            })
            .filter {
                case (tableName, operate, data) =>
                    ...
                    tableNames.contains(tableName) &&
                        ("insert" == operate || "update" == operate || "bootstrap-insert" == operate)
            }
            .foreachRDD(rdd => {
